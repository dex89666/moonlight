// R3FScene moved here from apps/web to avoid React version conflicts.
...
function Crystal({ position = [0, 0, 0], color = '#8b5cf6' }: any) {
  ...
  useFrame((state, dt) => {
    ...
    ref.current.rotation.y += 0.2 * dt * (hover ? 3 : 1)
    const target = new THREE.Vector3(hover ? 1.04 : 1, hover ? 1.04 : 1, hover ? 1.04 : 1)
    ref.current.scale.lerp(target, 0.06)
  })
