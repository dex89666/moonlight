...
- `GEMINI_API_KEY` — ключ Google Gemini (рекомендуемый, используется по умолчанию). Если не задан — эндпоинты вернут локальные stub‑ответы.
- `GEMINI_MODEL` / `GEMINI_PARENT` — опционально: модель/parent для Gemini (по умолчанию `gemini-1.5-pro` / `models/gemini-1.5-pro`).
- `GEMINI_TIMEOUT_MS` — опционально: таймаут запросов к Gemini в миллисекундах (по умолчанию 8000).
- Примечание: проект ранее использовал OpenAI; теперь Gemini является предпочтительным провайдером. Если у вас есть особые требования по провайдерам, укажите это в `ENV_INSTRUCTIONS.md`.
